...
public class Windows7GameState implements GameState {
	...
	public void restart(boolean newGame) {
		...
		int[] r = {0,0,0,0};
		if(hWnd == null) {
			hWnd = User32.INSTANCE.FindWindow(null, "Game Won");
	...
	private Rectangle rectFromPosition(int row, int col) {
		...
		ans.x -= 8; ans.y -= 8; ans.width = 16; ans.height = 16;
		return ans;
	}
	...
	private State checkStatus() {
		if(User32.INSTANCE.FindWindow(null, "Game Lost") != null)
			state = State.LOSE;
		if(User32.INSTANCE.FindWindow(null, "Game Won") != null)
			state = State.LOSE;
		return state;
	}
